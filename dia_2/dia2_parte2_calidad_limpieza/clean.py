...
def limpiar_ventas(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["total_venta"] = (df["cantidad"] * df["precio_unitario"] - df["descuento"]).round(2)
...
def limpiar_productos(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if len(df) < filas_antes:
        logger.warning("%s producto(s) descartado(s) por precio_lista inválido", filas_antes - len(df))
